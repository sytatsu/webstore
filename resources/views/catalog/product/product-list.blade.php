            ...
            <x-slot name="content">
                @foreach($products as $product)
                    <x-table.row checkbox="true">
                        <td>
                            <a class="w-full h-full avenir-bold underline text-secondary hover:text-secondary-dark" href="{{ route('catalog.products.show', $product) }}">{{ $product->name }}</a>
                        </td>
                        <td>{{ $product->productType->translation() }}</td>
                        <td>
                            @if($product->brand)
                                <a class="hover:underline" href="{{ route('catalog.brands.show', $product->brand) }}">{{ $product->brand->name }}</a>
                            @else
                        ...
                        <td>
                            @if($product->category)
                                <a class="hover:underline" href="{{ route('catalog.categories.show', $product->category) }}">{{ $product->category->name }}</a>
                            @else
                                <span class="text-sm rounded-lg py-2 px-3 bg-red-500 text-white"><i class="fa fa-warning pr-1"></i>N/A</span>
                            @endif
                        </td>
                        ...
                        <x-slot name="actions">
                            <x-table.partials.action :href="route('catalog.products.show', $product)">
                                <i class="fa fa-eye w-6"></i>Show
                            </x-table.partials.action>

                            <x-table.partials.action :href="route('catalog.products.edit', $product)">
