...
use App\Http\Requests\Catalog\Brand\BrandDeleteRequest;
use App\Http\Requests\Catalog\Brand\BrandStoreRequest;
use App\Http\Requests\Catalog\Brand\BrandUpdateRequest;
...
class BrandController extends Controller
{
    ...
    {
        return view(view: "catalog.brand.brand-list", data: [
    ...
    {
        return view(view: "catalog.brand.brand-create");
    }
    ...
    {
        return view(view: 'catalog.brand.brand-show', data: [
            'brand' => $brand,
        ]);
    }
    ...
    {
        return view(view: 'catalog.brand.brand-edit', data: [
            'brand' => $brand,
            'action' => $request->get('action'),
        ]);
    }
    ...
    public function delete(BrandDeleteRequest $request): RedirectResponse
    {
        ...
        return Redirect::route('catalog.brands.list');
    }
    ...
    private function saveAndAction(?string $action, Brand $brand): RedirectResponse
    {
        ...
        return match ($action) {
            SaveAndAction::CREATE_AGAIN => Redirect::route('catalog.brands.create'),
            SaveAndAction::TO_OVERVIEW => Redirect::route('catalog.brands.list'),
            SaveAndAction::TO_MODEL => Redirect::route('catalog.brands.show', $brand),
            default => null
        } ?? Redirect::route('catalog.brands.show', $brand); // Redirect fallback
    }
}
